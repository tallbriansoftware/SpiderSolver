...
// Tests if a given move (type/function) can
// be done between the source and destination stacks.
using MoveTestFunction = std::function<int(const SpiderStack&, const SpiderStack&)>;
...
namespace {
    ...
    int ScanMoveTestFunc_DestStack(
        std::vector<MoveSingle>& moves,
